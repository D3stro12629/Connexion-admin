                    ...
                    <div class="title-stack">
                        <h1>
                            ...
                            <span class="count-chip" v-if="!categoryStore.isLoading">
                                {{ categoryStore.categories?.length || 0 }} សរុប
                            </span>
                        </h1>
                        <p class="subtitle-text">រៀបចំ និងគ្រប់គ្រងការចាត់ថ្នាក់ផលិតផលរបស់អ្នក</p>
                    </div>
                    ...
                    <div class="header-actions">
                        <div class="action-group">
                            <div class="search-wrapper">
                                ...
                                <input v-model="searchQuery" type="text" placeholder="ស្វែងរកប្រភេទ..." />
            ...
            <section class="stats-row">
                <template v-if="categoryStore.isLoading">
                    <div v-for="i in 2" :key="'stat-s-' + i" class="stat-card">
                        ...
                        <div class="stat-data">
                            <BaseSkeleton width="60px" height="12px" radius="4px"
                                ...
                                <tr v-for="i in 6" :key="'table-s-' + i">
                                    ...
                                    <td class="text-end">
                                        <div class="action-btns">
                                            <BaseSkeleton v-for="b in 3" :key="b" width="36px" height="36px"
                                                radius="10px" />
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <div class="modal-surface compact-modal">
                            ...
                            <div class="modal-content compact-padding">
                                <div class="modal-header-clean">
                                    ...
                                    <h2>{{ isEditing ? 'កែប្រែប្រភេទ' : 'បន្ថែមប្រភេទ' }}</h2>
                                </div>
                                <div class="form-body">
                                    <div class="input-group-custom">
                                        <label>ឈ្មោះ</label>
                                        <input v-model="form.name" placeholder="e.g. Science Department"
                                            :class="{ 'has-error': errors.name }" @blur="validate('name')" />
                                        <span v-if="errors.name" class="error-msg">{{ errors.name }}</span>
                                    </div>
                                    <div class="image-upload-zone" @click="triggerFileInput">
                                        <i class="bi bi-image me-2"></i>
                                        <span>{{ form.image ? 'ប្តូររូបភាព' : 'ដាក់រូបភាព' }}</span>
                                    </div>
                                ...
                                <div class="modal-footer-refined">
                                    ...
                                    <button class="btn-primary" :disabled="categoryStore.isProcessing"
                                        @click="saveCategory">
                                        <span v-if="categoryStore.isProcessing"
                ...
                <Transition name="premium-modal">
                    <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
                        <div class="modal-surface compact-modal">
                            ...
                            <div class="modal-content compact-padding">
                                <div class="modal-header-clean">
                                    <div class="icon-circle compact danger"><i
                                ...
                                <div class="modal-footer-refined">
                                    ...
                                    <button class="btn-danger" :disabled="categoryStore.isProcessing"
                                        @click="confirmDelete">
                                        <span v-if="categoryStore.isProcessing"
                                            class="spinner-border spinner-border-sm me-2"></span>
                                        យល់ព្រម</button>
                            ...
                            <div class="modal-content compact-padding">
                                <div class="modal-header-clean">
                                    <div class="icon-circle compact detail">
                                        <img v-if="selectedItem?.image" :src="selectedItem.image"
                                            class="circle-img-preview" />
                                        <i v-else class="bi bi-info-circle-fill"></i>
                                    </div>
                                    <h2>ព័ត៌មានលម្អិត</h2>
                                </div>
                                <div class="info-grid-simple">
                                    <div class="info-box full"><label>ឈ្មោះ</label>
                                ...
                                <div class="modal-footer-refined">
                                    <button class="btn-secondary full-width"
                                        @click="showDetailsModal = false">បិទ</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
const saveCategory = async () => {
    ...
    // IMAGE VALIDATION
    if (!isEditing.value && !form.image) {
        toast.error('សូមដាក់រូបថត')
        return
    }
...
.input-group-custom input.has-error {
...
:deep(.pagination-right .is-active button) {
    ...
    /* Subtle glow to show it's selected */
}
