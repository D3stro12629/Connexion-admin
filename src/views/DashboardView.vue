    ...
    <DashboardLayout>
        <div class="premium-shell">
            ...
            <section class="stats-row">
                ...
                <template v-else>
                    <!-- v-for changed to use liveStatCards -->
                    <div v-for="card in liveStatCards" :key="card.label" class="stat-card fade-in">
            ...
            <main class="bento-grid-custom">
                <!-- Main Chart (FIXED: Now passes the correctly formatted live data) -->
                <div class="content-wrapper bento-box main-chart-wrapper">
                    <BarChart 
                        title="Category Distribution" 
                        subtitle="Total Items by category" 
                        icon="bi-graph-up"
                        :data="liveChartData" 
                ...
                <!-- RECENT SECTIONS (FIXED: Now points to liveRecentSections) -->
                <div v-for="section in liveRecentSections" :key="section.title" class="bento-box recent-card">
                    <div class="box-header-flex">
                        ...
                        <button v-if="!dashboardStore.isLoading" class="view-all-btn fade-in" @click="router.push(section.route)">
...
const handleQuickAction = (label) => {
    const routes = { 'Add Skill': '/skills', 'Add School': '/schools', 'Add Degree': '/degrees', 'Add Subject': '/subjects', 'Add Category': '/category' }
...
.skeleton-text::after, .skeleton-box::after, .skeleton-circle::after, .skeleton-circle-sm::after, .skeleton-card::after, .skeleton-action-tile::after, .skeleton-row-premium::after { content: ""; position: absolute; top: 0; right: 0; bottom: 0; left: 0; transform: translateX(-100%); background: linear-gradient(90deg, transparent, rgba(255,255,255,0.04), transparent); animation: shimmer 1.8s infinite; }
...
.skeleton-action-tile { display: flex; align-items: center; padding: 0.8rem; border-radius: 16px; background: #18181b; border: 1px solid var(--border); height: 60px; }
...
.skeleton-row-premium { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 1rem; border-radius: 12px; height: 46px; margin-bottom: 8px; background: rgba(255,255,255,0.02); }
.main-text-skel { width: 50%; height: 12px; }
.side-text-skel { width: 20%; height: 10px; }
.fade-in { animation: fadeInContent 0.5s ease forwards; }
@keyframes fadeInContent { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }
.premium-shell { --bg: #09090b; --surface: #121212; --border: #1f1f1f; --accent: #6366f1; --text-main: #fafafa; --text-muted: #71717a; background: var(--bg); color: var(--text-main); font-family: 'Plus Jakarta Sans', sans-serif; min-height: 100vh; padding: 2.5rem; overflow-x: hidden; }
...
.recent-icon-circle { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1rem; flex-shrink: 0; }
.actions-list { display: flex; flex-direction: column; gap: 10px; margin-top: 1rem; }
.action-tile-premium { display: flex; align-items: center; background: #18181b; border: 1px solid var(--border); border-radius: 16px; padding: 0.8rem; cursor: pointer; transition: 0.2s; color: white; width: 100%; text-align: left;}
